# temporary bug fix
nodal_adj <- function (lambp, lambN, lambph, indegree = TRUE, outdegree = TRUE)
{
    ...
    hc <- TideHarmonics::harmonics$name # bug fix so references harmonics in TideHarmonics
    if (indegree) {
        lambN <- lambN * pi/180
        lambp <- lambp * pi/180
    ...
    c2p <- cos(2 * lambp)
    s2p <- sin(2 * lambp)
    ones <- c("aSa", "Sa", "Ssa", "Sta", "pi1", "P1", "xS1",
        "S1", "aS1", "psi1", "NA2", "aNA2", "NB2", "NA2*", "MA2",
        "MB2", "MA2*", "T2", "S2", "R2")
    fn[, ones] <- 1
    un[, ones] <- 0
    ones <- c("MSm", "Mnum", "SM", "KOo", "MKo", "Snu", "SN",
        "MStm", "2SMN")
    fn[, ones] <- 1
    un[, ones] <- 0
    fn[, "Mm"] <- 1 - 0.1311 * c1 + 0.0538 * c2p + 0.0205 * cos(2 *
        lambp - lambN)
    un[, "Mm"] <- 0
    fn[, "Mf"] <- 1.084 + 0.415 * c1 + 0.039 * c2
    un[, "Mf"] <- -23.7 * s1 + 2.7 * s2 - 0.4 * s3
    fn[, "O1"] <- 1.0176 + 0.1871 * c1 - 0.0147 * c2 + 0.0014 *
        c3
    un[, "O1"] <- 10.8 * s1 - 1.34 * s2 + 0.19 * s3
    fn[, c("K1", "xK1")] <- 1.006 + 0.115 * c1 - 0.0088 * c2 +
        6e-04 * c3
    un[, c("K1", "xK1")] <- -8.86 * s1 + 0.68 * s2 - 0.07 * s3
    fn[, "J1"] <- 1.0129 + 0.1676 * c1 - 0.017 * c2 + 0.0016 *
        c3
    un[, "J1"] <- -12.94 * s1 + 1.34 * s2 - 0.19 * s3
    fn[, "M2"] <- 1.0007 - 0.0373 * c1 + 2e-04 * c2
    un[, "M2"] <- -2.14 * s1
    fn[, "K2"] <- 1.0246 + 0.2863 * c1 + 0.0083 * c2 - 0.0015 *
        c3
    un[, "K2"] <- -17.74 * s1 + 0.68 * s2 - 0.04 * s3
    ss <- 2.783 * s2p + 0.558 * sin(2 * lambp - lambN) + 0.184 *
        s1
    cc <- 1 + 2.783 * c2p + 0.558 * cos(2 * lambp - lambN) +
        0.184 * c1
    fn[, c("M1B", "xM1B")] <- sqrt(cc^2 + ss^2)
    un[, c("M1B", "xM1B")] <- atan2(ss, cc) * 180/pi
    ss <- s1p + 0.2 * sin(lambp - lambN)
    cc <- 2 * c1p + 0.4 * cos(lambp - lambN)
    fn[, c("M1", "xM1", "aM1", "M1C")] <- sqrt(cc^2 + ss^2)
    un[, c("M1", "xM1", "aM1", "M1C")] <- atan2(ss, cc) * 180/pi
    ss <- -0.3593 * s2p - 0.066 * sin(2 * lambp - lambN) - 0.2 *
        s1
    cc <- 1 + 0.3593 * c2p + 0.066 * cos(2 * lambp - lambN) +
        0.2 * c1
    ...
    ss <- -0.2505 * s2p - 0.1102 * sin(2 * lambp - lambN) - 0.0156 *
        sin(2 * lambp - 2 * lambN) - 0.037 * s1
    cc <- 1 - 0.2505 * c2p - 0.1102 * cos(2 * lambp - lambN) -
        0.0156 * cos(2 * lambp - 2 * lambN) - 0.037 * c1
    ...
    fn[, "tau1"] <- fn[, "K1"]
    un[, "tau1"] <- un[, "K1"]
    fn[, c("Mqm", "eps2", "2N2", "mu2", "N2", "nu2", "lam2")] <- fn[,
        "M2"]
    un[, c("Mqm", "eps2", "2N2", "mu2", "N2", "nu2", "lam2")] <- un[,
        "M2"]
    fn[, c("2Q1", "sig1", "Q1", "rho1")] <- fn[, "O1"]
    un[, c("2Q1", "sig1", "Q1", "rho1")] <- un[, "O1"]
    fn[, "O2"] <- fn[, "O1"]^2
    un[, "O2"] <- 2 * un[, "O1"]
    fn2 <- fn[, c("M2", "S2", "T2", "N2", "K2", "L2", "R2", "nu2",
        "lam2", "Q1", "K1", "J1", "O1", "P1", "S1"), drop = FALSE]
    un2 <- un[, c("M2", "S2", "T2", "N2", "K2", "L2", "R2", "nu2",
        "lam2", "Q1", "K1", "J1", "O1", "P1", "S1"), drop = FALSE]
    colnames(fn2) <- colnames(un2) <- c("1M", "1S", "1T", "1N",
        "1K", "1L", "1R", "1V", "1D", "1q", "1k", "1j", "1o",
        "1p", "1s")
    fn2m <- fn2[, "1M"]
    un2m <- un2[, "1M"]
    fn2 <- cbind(fn2, `2M` = 2 * fn2m, `3M` = 3 * fn2m, `4M` = 4 *
        fn2m, `5M` = 5 * fn2m, `6M` = 6 * fn2m, `7M` = 7 * fn2m,
        `2S` = 2 * fn2[, "1S"], `3S` = 3 * fn2[, "1S"], `2N` = 2 *
            fn2[, "1N"], `3N` = 3 * fn2[, "1N"], `2k` = 2 * fn2[,
            "1k"], `3k` = 3 * fn2[, "1k"], `2K` = 2 * fn2[, "1K"],
        `2p` = 2 * fn2[, "1p"])
    un2 <- cbind(un2, `2M` = 2 * un2m, `3M` = 3 * un2m, `4M` = 4 *
        un2m, `5M` = 5 * un2m, `6M` = 6 * un2m, `7M` = 7 * un2m,
        `2S` = 2 * un2[, "1S"], `3S` = 3 * un2[, "1S"], `2N` = 2 *
            un2[, "1N"], `3N` = 3 * un2[, "1N"], `2k` = 2 * un2[,
            "1k"], `3k` = 3 * un2[, "1k"], `2K` = 2 * un2[, "1K"],
        `2p` = 2 * un2[, "1p"])
    nms <- sub("x", "", grep("\\.", hc, value = TRUE))
    pos <- substr(nms, 1, regexpr("\\.", nms) - 1)
    neg <- substr(nms, regexpr("\\.", nms) + 1, regexpr("\\d{1,2}$",
        nms) - 1)
    ...
    fn[, grep("\\.", hc)] <- exp(sapply(pos, tmpfn) + sapply(neg,
        tmpfn))
    un[, grep("\\.", hc)] <- sapply(pos, tmpun) - sapply(neg,
        tmpun)
